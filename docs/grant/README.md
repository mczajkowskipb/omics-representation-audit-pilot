...
2. `SONATA_BIS16_STRATEGIC_DECISION_MODIFY_PL.md` - critical comparison of the
   TRPP-centred and broad representation-centred variants, the `MODIFY`
   decision, central question, hypotheses, boundaries, and reviewer-facing
   narrative;
3. `SONATA_BIS16_DEFAULTS_FROZEN.md` - approved working choices and the items
   that still require factual input;
4. `SONATA_BIS16_SHORT_DESCRIPTION_EN.md` - self-contained short description,
   laid out as five scientific pages plus references;
5. `SONATA_BIS16_DETAILED_DESCRIPTION_EN_DRAFT.md` - self-contained detailed
   description, laid out as thirteen scientific pages plus references;
6. `SONATA_BIS16_POPULAR_SUMMARY_PL.md` and
   `SONATA_BIS16_POPULAR_SUMMARY_EN.md` - one-page public summaries;
7. `SONATA_BIS16_DATASET_AUDIT.md` - default lung and colorectal cohorts,
   inclusion checks, and invalidating risks;
8. `SONATA_BIS16_SCIENTIFIC_CORE_PL.md` - Polish scientific thesis, hypotheses,
   work packages, evaluation, and risks;
9. `SONATA_BIS16_OSF_STARTER_PL_EN.md` - title, keywords, abstract, task names,
   and formal checklist;
10. `SONATA_BIS16_PRELIMINARY_RESULTS_EN.md` - insertion-ready pilot evidence;
11. `SONATA_BIS16_CLAIM_BOUNDARIES.md` - hard overclaiming boundary;
12. `CLAIMS_EVIDENCE.json` - machine-readable values linked to PILOT-019.
